...
it. He found out it was gold and became rich because he had {num} pieces of gold.'''
    ...
    submit_btn = Button(ml1_wn, text="Submit", background="SteelBlue", font=('Times', 12),command=lambda:finish_madlib(ml1_wn, animal_entry.get(), parent_opt.get(), month_opt.get(),path_opt.get(), movement_opt.get(), num_entry.get()))
...
Then it went back into the woods and waited for its next victim to emerge.'''
    ...
    # Creating a 'Submit' button to add all the inputs to the final story
    submit_btn = Button(ml2_wn, text="Submit", background="SteelBlue", font=('Times', 12),command=lambda: finish_madlib(ml2_wn, boy_name_entry.get(), cloth_entry.get(),creature_entry.get(), adj_entry.get(), village_entry.get(), exclamation_entry.get(), fav_thing_entry.get()))
...
Label(root, font=("Comic Sans MS", 16), text='Mad Libs Generator').place(x=0, y=0)
Label(root, font=("Comic Sans MS", 16), text='Have fun!').place(x=130, y=35)

ml1 = Button(root, text='The Gold', font=("Comic Sans MS", 16), command=lambda: first_madlib(root),
             bg='LightSkyBlue')
ml1.place(x=125, y=90)

ml2 = Button(root, text='One Dark, Stormy Night', font=("Comic Sans MS", 16), command=lambda: second_madlib(root),
             bg='LightSkyBlue')
ml2.place(x=50, y=150)

ml3 = Button(root, text='The Ring', font=("Comic Sans MS", 16), command=lambda: third_madlib(root),
             bg='LightSkyBlue')
